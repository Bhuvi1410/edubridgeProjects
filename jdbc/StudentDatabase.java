...
public class StudentDatabase {
	...
	void toAdd(Connection connection) throws NullPointerException, SQLException{
		PreparedStatement insertStatement = connection.prepareStatement("INSERT INTO student_detail values(?, ?, ?, ?)");
	...
	void toDisplaybyId(Connection connection) throws NullPointerException, SQLException {
		PreparedStatement prepstmnt = connection.prepareStatement("SELECT * FROM student_detail WHERE id = ? ");  
		...
		while(searchRecord.next()) {
			System.out.println(searchRecord.getInt(1)+" "+searchRecord.getString(2)+" "+searchRecord.getString(3)+" "+searchRecord.getInt(4));
		}
	...
	void toUpdate(Connection connection) throws NullPointerException, SQLException{
        ...
        PreparedStatement updateStmt = connection.prepareStatement("UPDATE student_detail SET name = ? WHERE id = ?");
	...
	void toDelete(Connection connection) throws NullPointerException, SQLException{
		PreparedStatement pstmt = connection.prepareStatement("DELETE FROM student_detail WHERE id = ? ");
